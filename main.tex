...
\fancyhead[R]{\thepage}
\fancyhead[L]{St. Petersburg Campus of Higher School of Economics. SPb HSE: Abstract Economists}
...
\inputminted[linenos, breaklines]{c++}{code/kto.cpp}
\inputminted[linenos, breaklines]{c++}{code/fft.cpp}
\inputminted[linenos, breaklines]{c++}{code/gauss.cpp}
\inputminted[linenos, breaklines]{c++}{code/stable_gauss.cpp}
...
\inputminted[linenos, breaklines]{c++}{code/global_cut.cpp}
...
\inputminted[linenos, breaklines]{c++}{code/aho.cpp}
\inputminted[linenos, breaklines]{c++}{code/pollard.cpp}
...
\inputminted[linenos, breaklines]{c++}{code/planar.cpp}
...
{\bf Пентагональная теорема Эйлера}:\\
$\prod\limits_{n=1}^{\infty} (1 - x^n) = 1 + \sum\limits_{k=1}^{\infty} (-1)^k \left(x^{k(3k-1)/2} + x^{k(3k+1)/2} \right)$\\
